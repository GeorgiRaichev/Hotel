...
void FileManager::loadRooms(RoomManager& roomManager) {
	...
	while (file.getline(line, sizeof(line))) {
		...
		if (!room) {
			std::cerr << "Unknown room type in line: " << line << std::endl;
			continue;
		}
...
void FileManager::saveRooms(const RoomManager& roomManager) {
	...
	for (int i = 0; i < roomManager.size(); ++i) {
		...
		if (!room) {
			std::cerr << "NULL room pointer at index " << i << " while saving rooms.\n";
			continue;
		}
...
Date parseDate(const char* str) {
	int d = 0, m = 0, y = 0;
	if (std::sscanf(str, "%d.%d.%d", &d, &m, &y) != 3) {
		std::cerr << "Invalid date format: " << str << std::endl;
		return Date();
	}
	return Date(d, m, y);
}
